...
namespace Greenergy.Database
{
    public class PrognosisRepository : IPrognosisRepository
    {
        ...
        public async Task<List<PrognosisDataMongo>> GetPrognoses(string region, int hours, DateTime? startNoEarlierThanUTC, DateTime? finishNoLaterThanUTC)
        {
            startNoEarlierThanUTC = startNoEarlierThanUTC ?? DateTime.UtcNow;
            finishNoLaterThanUTC = finishNoLaterThanUTC ?? DateTime.MaxValue.ToUniversalTime();

            // if (startNoEarlierThanUTC == DateTime.MinValue)
            // {
            //     startNoEarlierThanUTC = DateTime.UtcNow;
            // }

            // if (finishNoLaterThanUTC == DateTime.MinValue)
            // {
            ...
            var prognoses = await _context.PrognosisCollection
                    .Find(p => p.EmissionTimeUTC.CompareTo(startNoEarlierThanUTC) >= 0
                              ...
                              && p.Region.Equals(region))
                    .SortBy(p => p.EmissionTimeUTC)
                    .ToListAsync();

            if (prognoses?.Count() == 0) throw new NotSupportedException("No prognosis data");
